...
queries_path = Path("/Users/hutch_ii/Projects/werewolf-arena-leaderboard/tests/queries.json")
results_dir = Path("/Users/hutch_ii/Projects/werewolf-arena-leaderboard/results")
...
if not result_files:
        ...
        try:
            ...
            if 'results' not in data:
                ...
                if not isinstance(data['results'], list):
                    ...
                    # Check that results contain valid game objects
                    for idx, result in enumerate(data['results']):
                        if not isinstance(result, dict):
                            validation_errors.append(f"{result_file.name}: results[{idx}] must be an object")
                        elif 'winner' not in result:
                            validation_errors.append(f"{result_file.name}: results[{idx}] missing 'winner' key")
    ...
    if validation_errors:
        ...
        print("    \"results\": [ { \"winner\": \"...\", \"rounds_played\": N, ... } ]")
        print("  }")
        print()
        exit(1)
